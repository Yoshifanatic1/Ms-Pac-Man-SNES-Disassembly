...
set PATH="../../Global"
set Input1=
set ROMName=MSPM.sfc
set MemMap=lorom

setlocal EnableDelayedExpansion

echo To fully extract all files for supported ROMs, you'll need one of the following ROMs in each group:
echo - Graphics: MSPM (USA)
echo - Samples:  MSPM (USA)
echo - Music: MSPM (USA)
echo.

:Start
echo Place a headerless MSPM ROM named %ROMName% in this folder, then type the number representing what version %ROMName% is.
echo 0 = MSPM (USA)
echo 1 = MSPM (PAL)
...
echo You need to place a MSPM ROM named %ROMName% in this folder before you can extract any assets^^!
goto :Mode
...
if %PointerSet% equ 6 goto :MoveCGFX
